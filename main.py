...
def new_reminder(update, context):
    update.message.reply_text("Whose birthday shall I remind you off?")
    return 1
...
def get_date(update, context):
    ...
    cursor.execute(
        "INSERT INTO birthday_reminder (user_id, name, date, reminded) VALUES (?, ?, ?, ?)",
...
def do_reminder():
    while True:
        ...
        cursor.execute(
            "SELECT * FROM main.birthday_reminder WHERE strftime('%d', date) = strftime('%d', 'now')"
        ...
        for row in rows:
            ...
            updater.bot.send_message(
                chat_id=user_id, text=f"It's {name}'s birthday today!"
            )
            cursor.execute(
                "UPDATE birthday_reminder SET reminded = 1 WHERE id = ?", (row_id,)
            )
            conn.commit()
